...
class TrajectoryValidator:
    ...
    def __init__(self, config: ValidatorConfig):
        ...
        logger.info("=" * 60)
        logger.info("TrajectoryRL Validator v4.0.0")
        logger.info("=" * 60)
    ...
    def _build_report_metadata(
        ...
        for uid, commitment in active.items():
            ...
            # Build scenario details from EMA state
    ...
    async def _compute_and_set_weights(self, current_block: int):
        ...
        if not costs:
            logger.warning("No miners with cost data")
        ...
        if not costs:
            logger.warning("All miners excluded by NCD dedup or missing cost data")
            self._build_report_metadata(active, {}, {}, {})
            await self._set_fallback_weights()
            return

        num_active = len(costs)
