...
> v0.20: Implementation complete. Validator integration via docker-run CLI (no pip dependency on trajectory-sandbox). Two scenarios live: incident_response (22 criteria) + morning_brief (18 criteria). Decoupled architecture: updating scenarios = rebuild sandbox image only. Sandbox version drives scoring_version (v1.0.0 → scoring_version=1). Pack format: SKILL.md only, no AGENTS.md/tool_policy. Verified: live Hermes Agent test 22/22, pressure test +39-50pp SKILL.md advantage.
