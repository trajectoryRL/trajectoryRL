...
> **Dynamic epoch (2026-05-10).** Under v6, an epoch may finalise before `end_block` once every operator-curated whitelist validator has submitted and a minimum block floor has elapsed (`EPOCH_MIN_BLOCKS`, default 30, server-side). The daemon already handles a closed epoch correctly — `POST /score` returns `409 epoch is finalized` and the next `/api/v2/epoch/current` poll returns the new epoch. The daemon now polls `/api/v2/epoch/current` at a fixed cadence of `_EPOCH_POLL_INTERVAL` (10 s) and does **not** derive a long sleep from `remaining_blocks * 12s` — under early-finalise that would overshoot the boundary. After an eval finishes, the daemon simply waits the same 10 s before its next tick. See [API.md → Dynamic Epoch (early finalise)](API.md#dynamic-epoch-early-finalise).
